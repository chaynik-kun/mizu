...
import mizu.composeapp.generated.resources.info_lyrics_provider
...
import chaynik.mizu.util.core.calculateWordProgress
import kotlin.math.abs
import kotlin.time.Duration
import kotlin.time.Duration.Companion.milliseconds

internal fun lyricsSeekPosition(timestamp: Duration?): Duration? =
	timestamp?.coerceAtLeast(Duration.ZERO)
...
fun LyricsScreenContent(
	...
	LazyColumn(
		...
		itemsIndexed(lyrics) { index, line ->
			...
			val seekPosition = lyricsSeekPosition(line.time)
			...
			LyricsScreenKaraokeText(
				...
				onClick = {
					if (isSelecting) {
						...
						player.debugLyricsTap(songId, index, line.text, line.time, duration)
